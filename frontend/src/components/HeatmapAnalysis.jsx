...
const HeatmapAnalysis = ({ heatmapData, taskID }) => {
  if (!heatmapData || Object.keys(heatmapData).length === 0) {
    return (
      <div className="p-4 border border-gray-200 rounded-lg mb-4 bg-gray-50">
        <h3 className="text-lg font-medium mb-2">Heatmap Analysis</h3>
  ...
  return (
    <div className="p-4 border border-gray-200 rounded-lg mb-4 bg-gray-50">
      <h3 className="text-lg font-medium mb-4">Heatmap Analysis</h3>
      
      <div className="grid grid-cols-2 gap-4">
        <div className="p-3 bg-white rounded-lg shadow-sm">
          <p className="text-sm text-gray-500">Peak Movement Time</p>
          <p className="text-xl font-semibold">{formatTime(peak_movement_time)}</p>
        </div>
        
        <div className="p-3 bg-white rounded-lg shadow-sm">
          <p className="text-sm text-gray-500">Average Movement Intensity</p>
          <p className="text-xl font-semibold">{average_intensity ? average_intensity.toFixed(2) + '%' : 'N/A'}</p>
        </div>
        
        <div className="p-3 bg-white rounded-lg shadow-sm">
          <p className="text-sm text-gray-500">Movement Duration</p>
          <p className="text-xl font-semibold">{formatTime(movement_duration)}</p>
        </div>
